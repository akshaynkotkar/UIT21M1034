...
function AllProducts() {
  ...
  const [selectedCompany, setSelectedCompany] = useState('PC'); // Default value
  const [selectedCategory, setSelectedCategory] = useState('TV'); // Default value
  const [minPrice, setMinPrice] = useState(1); // Default value
  const [maxPrice, setMaxPrice] = useState(10000); // Default value
  const [discount, setDiscount] = useState(0); // Default value
  ...
  useEffect(() => {
    fetchProducts();
  }, [selectedCompany, selectedCategory, minPrice, maxPrice, discount, currentPage, sortOrder, sortBy]);

  const fetchProducts = () => {
    const startIndex = (currentPage - 1) * productsPerPage;
    const endIndex = startIndex + productsPerPage;
    
    fetch(`http://127.0.0.1:8000/api/categories/${selectedCategory}/products?company=${selectedCompany}&top=${endIndex}&minPrice=${minPrice}&maxPrice=${maxPrice}&discount=${discount}&sort=${sortBy}&order=${sortOrder}`)
